...
def analyzeO2_with_tumor(f):
    ...
    with mpl_utils.PdfWriter(outfn) as pdfwriter:
      ...
      img = plotBulkTissue.imshow(ax, plotBulkTissue.imslice(o2), ld, vmin = 0., vmax = 9., cmap = matplotlib.cm.Reds, worldscaling = 1.e-3)
      ...
      ax = fig.add_axes([0.05, 0.05, 4.2/5, 4.2/5])
      img = plotBulkTissue.imshow(ax, plotBulkTissue.imslice(consumption), ld, vmin = consumption.min(), vmax = consumption.max(), cmap = matplotlib.cm.Reds, worldscaling = 1.e-3)
      plotBulkTissue.colorbar(ax.get_figure(), ax, img)
      plotBulkTissue.contour(ax, plotBulkTissue.imslice(dist_tumor), ld, levels = [0.], colors = ['k'], worldscaling = 1.e-3)
...
if __name__ == "__main__":
  import argparse
  parser = argparse.ArgumentParser(description='Analyze simple Oxygen from BulkTissue tumor.')  
  parser.add_argument('bulkTumorFileNames', nargs='+', type=argparse.FileType('r'), default=sys.stdin, help='bulk tumor files to calculate')
  ...
  for fn in goodArguments.bulkTumorFileNames:
    f = h5py.File(fn.name, 'r')

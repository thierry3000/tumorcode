...
if __name__ == "__main__":
  ...
  #filenames = args[:]
  #pattern = 'adaption/vessels_after_adaption'
  #pattern = 'vessels'
  fn_measure = "all_data.h5"
  quantities = 'flow radius shearforce'
  for quantity in quantities.split():
    with PdfPages('all_densities'+quantity+'.pdf') as pdf:
      DoReadIn(filenames, pattern, fn_measure, pdf, quantity)
